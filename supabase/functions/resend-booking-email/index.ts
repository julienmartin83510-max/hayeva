...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (emailType === 'received') {
      ...
      bodyHtml = `
        <p style="margin-top:0;">Merci d'avoir choisi HAYEVA.</p>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:14px;margin-top:16px;">
          ...
          <tr><td style="padding:6px 0;color:#5B6B78;">Date demandée</td><td style="padding:6px 0;">${fmtDate(booking.date)}</td></tr>
          <tr><td style="padding:6px 0;color:#5B6B78;">Créneau</td><td style="padding:6px 0;">${(booking.start_time || '').slice(0, 5)}</td></tr>
      ...
      bodyHtml = `
        <p style="margin-top:0;">Bonne nouvelle, votre rendez-vous HAYEVA est <strong>confirmé</strong>.</p>
        <table style="width:100%;border-collapse:collapse;font-size:14px;margin-top:16px;">
          <tr><td style="padding:6px 0;color:#5B6B78;width:150px;">Prestation</td><td style="padding:6px 0;font-weight:600;">${escapeHtml(serviceName)}</td></tr>
      ...
      bodyHtml = `<p style="margin-top:0;">Votre demande de rendez-vous n'a malheureusement pas pu être retenue. N'hésitez pas à nous contacter ou à effectuer une nouvelle demande pour un autre créneau.</p>`;
    }
    ...
    const html = `
      <div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;margin:0 auto;color:#16222c;">
        ...
        ${bodyHtml}
        ${booking.customer_user_id ? `<p style="margin-top:22px;"><a href="${CLIENT_PANEL_URL}" style="display:inline-block;background:#1AA6EE;color:#ffffff;text-decoration:none;padding:12px 22px;border-radius:999px;font-weight:600;font-size:14px;">Voir mon espace</a></p>` : ''}
