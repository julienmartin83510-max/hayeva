-- ============================================================
-- Corrige verify_distance_quote() : hmac() de pgcrypto exige des arguments
-- bytea, pas text — "function hmac(text, text, unknown) does not exist"
-- constaté lors du premier test réel (réservation avec une adresse à Nice).
-- Même correctif nécessaire côté Edge Function ? Non : calculate-travel-
-- distance utilise l'API Web Crypto de Deno (crypto.subtle), qui ne
-- présente pas ce problème — seule la vérification côté SQL était
-- affectée.
